...
export default function ScreenerPage() {
  ...
  const [meta, setMeta]           = useState({ total: 0, cached: 0, universe: 0 })
  ...
  const load = useCallback(async (f: Filters, p: number) => {
    setLoading(true)
    const ps = new URLSearchParams({ page: String(p), limit: '25' })
    if (f.q)        ps.set('q', f.q)
    ...
    try {
      ...
      setMeta({ total: d.total ?? 0, cached: d.cached ?? 0, universe: d.universe ?? 0 })
  ...
  const FilterPanel = () => (
    <div className="space-y-4">
      ...
      {[
        { label: 'Ticker / Name', placeholder: 'AAPL…', key: 'q', upper: true },
      ].map(({ label, placeholder, key, upper }) => (
        <div key={key}>
          ...
          <input className="w-full bg-[#0a0e1a] border border-[#2d3748] rounded-md px-3 py-2 text-sm text-white placeholder-[#6b7280] focus:border-blue-500 focus:outline-none"
            ...
            onChange={e => setFilters(f => ({ ...f, [key]: upper ? e.target.value.toUpperCase() : e.target.value }))} />
        </div>
      ))}
      ...
      <div>
        <label className="text-xs text-[#6b7280] mb-1 block">Min score: <span className="text-white font-bold">{filters.minScore}</span></label>
      ...
      {[
        ...
        { label: 'Min MoS (%)', key: 'minMoS', placeholder: 'e.g. 20' },
      ...
      <div className="flex flex-col gap-2 pt-1">
        ...
        <button onClick={() => { setFilters(DEFAULT); setApplied(DEFAULT); setPage(1) }}
    ...
    <div className="flex gap-5">
      ...
      <div className="flex-1 min-w-0 space-y-3">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-xs text-[#6b7280] mt-0.5">
              {loading
                ? 'Loading…'
                : `${meta.total} results · ${meta.cached}/${meta.universe} cached · sorted by score`}
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => load(applied, page)} title="Refresh"
        ...
        <div className="bg-[#141824] border border-[#2d3748] rounded-xl overflow-hidden">
            ...
            <table className="w-full min-w-[1100px] text-sm">
              ...
              <tbody className="divide-y divide-[#1e2332]">
                {loading && companies.length === 0 ? (
                  ...
                  <tr><td colSpan={15} className="py-10 text-center text-[#9ca3af]">No results.</td></tr>
                ) : sorted.map(co => (
                  <tr key={co.ticker} className="hover:bg-[#1e2332] transition-colors">
                    ...
                    <td className="px-3 py-2.5">
                      <ScoreGauge score={co.score} size="sm" showLabel={false} />
                    </td>
                    <td className="px-3 py-2.5 hidden lg:table-cell">
                      <CategoryBadge category={co.category} compact />
                    </td>
          ...
          {/* Pagination */}
          {meta.total > 25 && (
            <div className="flex items-center justify-between px-4 py-3 border-t border-[#2d3748] text-sm text-[#9ca3af]">
              <span className="text-xs">Page {page} · {meta.total} results</span>
              <div className="flex gap-2">
                <button disabled={page === 1} onClick={() => setPage(p => p - 1)}
                  className="px-3 py-1 bg-[#1e2332] rounded-md disabled:opacity-30 hover:text-white transition-colors text-xs">
                  ← Prev
                </button>
                <button disabled={page * 25 >= meta.total} onClick={() => setPage(p => p + 1)}
                  className="px-3 py-1 bg-[#1e2332] rounded-md disabled:opacity-30 hover:text-white transition-colors text-xs">
                  Next →
                </button>
              </div>
        ...
        <p className="text-xs text-[#6b7280] text-center">
          {meta.cached < meta.universe
            ? `Cache warming: ${meta.cached}/${meta.universe} companies loaded · refresh to see more`
            : 'All companies loaded from cache'}
